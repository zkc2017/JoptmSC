...
double get_L2_norm_of_Bern_polynomial_vs_target_function(double(*fp1)(double), double(*fp2)(double, vector<double>), vector<double> Bern_coef_vec, int evaluating_point_number) {
    ...
    double step = 1 / double(n);
    double sum = 0;
    int sbs_len=pow(2,BIT_WIDTH)-1; //new-kcz
    for (int i = 1; i <= n; i++) {
        double v = double(i * step);
        double v2= double(round(v*sbs_len)/sbs_len); //new-kcz
        double r1 = (*fp1)(v);
        double r2 = (*fp2)(v2, Bern_coef_vec); //new-kcz
        sum += pow((r1 - r2), 2);
    }
    double result = sqrt(sum / double(n));
    return result;
...
//    in.open("./FVnm/cos(x).txt");
    ...
    if(dpsMax == startIndex){
        int num = 0;
        for(auto n = 1; n <= dpsMax; n++){  //degree
            int m = dpsMax - n;  //accuracy
            ...
            if (approx_error <= approx_error_bound) {
                ...
                appr.first = tempnm;
                appr.second = initialFV;
                lowerDegreeApprSet[0].push_back(appr);
            }
            num++;
