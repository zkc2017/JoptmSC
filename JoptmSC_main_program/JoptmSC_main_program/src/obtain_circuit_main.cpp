...
//    double approx_error_bound_ratio = 0.02;
	...
	switch(bm_id_under_test){
		...
		case 14:
			...
			break;
		case 15:
			fp_target_func = &bm15_target_func;
			break;
	}
    ...
    /*if((bm_id_under_test == 14) && (approx_error_bound_ratio_real == 0.05)){
    	timeBound = 300000u;
    }
    else if((bm_id_under_test == 11) && (approx_error_bound_ratio_real == 0.02)){
        	timeBound = 300000u;
    }
    else{
    	timeBound = 30000u;
...
//   double approx_error_bound = L2norm_of_target_function * approx_error_bound_ratio_real;
   double approx_error_bound = approx_error_bound_ratio_real;
   cout << "approx_error_bound = " << approx_error_bound << endl;

   auto start = chrono::system_clock::now();
   ApprInfoSet lowerDegreeApprSet;
   lowerDegreeApprSet = findAFV(prefix, approx_error_bound, fp_target_func, MDV, startIndex, bm_id_under_test, approx_error_bound_ratio_real);
...
//   		int mgs2[5] = {5, 8, 11, 12, 13};
   		int mgs2[2] = {-1,-1};//{12, 13};
   		vector <int> mgs_vec2(mgs2, mgs2+2);
   	...
   	else if(approx_error_bound_ratio == 0.05 || approx_error_bound_ratio == 0.04){
   		int mgs5[1] = {-1};//{16};
...
//			flag = 1;
	...
	int optimal_site = minPos - areaSet.begin(); //new-kcz
	...
	char sol[1000];
	 ...
	 strcat(sol, ".txt");
	 ofstream out(sol);
	 if(ASCPSet[optimal_site].first[0] + ASCPSet[optimal_site].first[1] < 4){
		 ASCPSet[optimal_site].first[1] = 4 - ASCPSet[optimal_site].first[0];
	 }
	 out << ASCPSet[optimal_site].first[0] << " " << ASCPSet[optimal_site].first[1]<< " " << minPos - areaSet.begin() << "." << endl; //new-kcz output n,m
