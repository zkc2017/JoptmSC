...
int main(int argc, char* argv[]){
    ...
    for(int bm_id_under_test=1;bm_id_under_test<15;bm_id_under_test++){
    	if(bm_id_under_test==12){
    		continue;
    	}
...
int pseudo_main(int argc, char* argv[]){
	...
	double approx_error_bound_ratio_real = approx_error_bound_ratio*2;
    ...
    while(whether_loop){
		int circuit_sig = obtain_circuit_main( bm_id_under_test, approx_error_bound_ratio, bit_width, simulation_num, approx_error_bound_ratio_real);
		double err_min = simulation_main( bm_id_under_test, approx_error_bound_ratio, bit_width, simulation_num);
		if(err_min<approx_error_bound_ratio){
			break;
		}
		if(err_min>approx_error_bound_ratio){
			approx_error_bound_ratio_real = approx_error_bound_ratio_real - (err_min - approx_error_bound_ratio);
		}
	}
	return 0;
}
...
double algorithm_search_minimum_error(int bit_width,one_die_vector_int circuit_information,three_die_vector_int information_combine,int simulation_num,double(*fp1)(double),int bm_id_under_test,double approx_error_bound_ratio){
	...
	while(whether_loop){
		...
		for(int i=0;i<permutation_bw_num;i++){ //algorithm-2-loop-1
			...
			double p1 = mt()*1.0/mt.max();
			configuration_temp[0] = round((LFSR_num-1)*p1);

			double p2 = mt()*1.0/mt.max();
	...
	for(int i=0;i<simulation_num;i++){
		...
		for(int j=0;j<verilog_circuit.size();j++){
			...
			case 19:
				...
				break;
			case 20:
				gate_output_bit = buf( len_len, gate_input_bit);
				break;
			case 21:
...
one_die_vector_bit buf(int len_len, two_die_vector_bit gate_input_bit){ //GATE buf        1       O=a;            PIN * NONINV 1 999 1.0 0.0 1.0 0.0 right
	one_die_vector_bit gate_out_bit;
	uint64_t bit;
	one_die_vector_int temp;
